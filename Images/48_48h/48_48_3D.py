...
data_0= np.array([io.imread('/home/marcos/Escritorio/TFG_peixe_cebra/Codigo/AppZebraFish/Images/48_48h/pez 48_0%i.tif' % i) for i in range(0, 10)])
data_0= np.concatenate((data_0,np.array([io.imread('/home/marcos/Escritorio/TFG_peixe_cebra/Codigo/AppZebraFish/Images/48_48h/pez 48_%i.tif' % i) for i in range(10, 40)])),axis=0)
data_0=data_0[:,:,:,1]
data_0=data_0.T
...
"""este for va en función del número de imágenes:40"""  
...
def volumen_voxels(imagen,umbral):
    nGFP=[0 for x in range(0,50/5+1)]
    meanGFP=[0 for x in range(0,50/5+1)]
    ndatos=0
    for umbral in range(0,55,5):
    ...
    f=open("/home/marcos/Escritorio/TFG_peixe_cebra/Codigo/AppZebraFish/Images/48_48h/datos2.txt","w")
    for nG in nGFP:	
    	f.write(str(nG))
    	f.write(" ")
    f.write("\n")	
...
threshold=0
vol=volumen_voxels(data_0,threshold)
vol=np.multiply(vol,data_0)
print "UMBRAL",threshold
f=open("/home/marcos/Escritorio/TFG_peixe_cebra/Codigo/AppZebraFish/Images/48_48h/datos2.txt","a")
f.write(str(list(zip(*np.where(vol>threshold)))))
f.close()
